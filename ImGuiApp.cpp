...
static void check_vk_result(VkResult err_)
{
    if (err_ == 0)
        return;
...
{
    tex_data->Channels = 1;
    tex_data->Width = crystal->_width;
    tex_data->Height = crystal->_height;

    void* image_data = (void*)(crystal->_crystalArray.data());
    ...
    // Create the Vulkan image.
    {
        ...
        info.extent.width = static_cast<uint32_t>(tex_data->Width);
        info.extent.height = static_cast<uint32_t>(tex_data->Height);
        info.extent.depth = 1;
        info.mipLevels = 1;
        info.arrayLayers = 1;
    ...
    // Release image memory NOT NEEDED FOR CRYSTAL AS IT READS DATA FROM THE VECTOR THAT IS STILL IN USE
    //free(image_data);
...
static void SetupVulkanWindow(VkSurfaceKHR surface, int width, int height)
{
    wd->Surface = surface;
...
void ImGuiApp::NewFrame(GLFWwindow *window){
...
void ImGuiApp::Shutdown(GLFWwindow *window) {
    // Cleanup
    err = vkDeviceWaitIdle(g_Device);
    check_vk_result(err);
...
ImGuiApp::ImGuiApp(){
    ...
    m_Window = glfwCreateWindow(1280, 720, "My Dear ImGui App", nullptr, nullptr);
